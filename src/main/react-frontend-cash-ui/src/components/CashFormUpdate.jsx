...
class CashFormUpdate extends Component {
  constructor(props) {
    super(props);
    this.state = {
      cashId: "",
  ...
  updateCashById = (e) => {
    e.preventDefault();
    axios
      .post("http://localhost:8080/cashService/updateCash", this.state)
      .then((response) => {
        console.log(response.data);
        this.state.showMe = false;
        alert("Cash details to updated successfully");
      })
      .catch((error) => {
        alert("Cash details are not updated");
        this.setState({
